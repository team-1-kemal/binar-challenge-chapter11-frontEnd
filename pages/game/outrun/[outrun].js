...
const Outrun = () => {
  ...
  const scoreHandler = () => {
    ...
    setPoint(value);
    setPlayed(localStorage.setItem("played6", true));

    let pointUser = user.point;
    pointUser += value;
    axios.put(`/game/${4}/${id}?point=${pointUser}&title=OUTRUN`).catch((err) => console.log(err));
    setLoading(true);
    setHidden("");
    setTimeout(() => {
      setLoading(false);
  ...
  return (
    <>
      <Head>
        <title>Game Nation - Outrun</title>
      </Head>
      <section className="flex items-center justify-center bg-[url('/asset/bg-outrun.png')] min-h-screen bg-cover bg-center">
        ...
        <div className="flex flex-col h-[400px] items-center text-center mb-[250px]">
          <img src="/asset/logo-outrun.png" className="h-[80px] w-auto mt-[100px]" />
          <button
          ...
          {loading ? (
            <div className="mt-[50px] ml-[5px]">
              <Loader color={"#850ec9"} loading={loading} size={55} aria-label="Loading Spinner" data-testid="loader" />
