...
const Sinvader = () => {
  ...
  const scoreHandler = () => {
    ...
    setPoint(value);
    setPlayed(localStorage.setItem("played4", true));

    let pointUser = user.point;
    pointUser += value;
    axios
      .put(`/game/${4}/${id}?point=${pointUser}&title=SPACE INVADERS`)
  ...
  return (
    <>
      <section className="flex items-center justify-center bg-[url('/asset/bg-sinvader.png')] min-h-screen bg-cover bg-center">
